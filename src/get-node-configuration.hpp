...
NetworkConfiguration networkConfiguration1 = {{}, {1}, {2}, {1, 1}};
...
LHRP_Node_Secure getNodeSecure(uint8_t netId, const array<uint8_t, 16> &key, NetworkConfiguration networkConfiguration)
{
    ...
    LHRP_Peer peer4 = {{0xEC, 0xE3, 0x34, 0xDB, 0x6A, 0xF4}, networkConfiguration.node4};
        ...
        peer1.mac[5] == mac[5])
    {
        return LHRP_Node_Secure(netId, key, {peer1, peer2, peer3});
             ...
             peer2.mac[5] == mac[5])
    {
        return LHRP_Node_Secure(netId, key, {peer2, peer1, peer4});
    }
             ...
             peer3.mac[5] == mac[5])
    {
        return LHRP_Node_Secure(netId, key, {peer3, peer1, peer2});
    }
    else
    {
        return LHRP_Node_Secure(netId, key, {peer4, peer2});
    }

    return LHRP_Node_Secure(netId, key, {peer1, peer2, peer3, peer4});
}
